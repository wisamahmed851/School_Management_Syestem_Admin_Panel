...
import { useRolePermissionsList } from "@/hooks/use-role-permissions";
...
export default function RolePermissionsPage() {
  ...
  const { data: role, isPending: rolePending, isError: roleError } = useRole(id);
  // Full list — filter client-side (no filter param exists on this endpoint)
  const { data: allAssignments = [], isPending: assignPending } = useRolePermissionsList();
  const { data: allPermissions = [], isPending: permsPending } = usePermissionsList();

  const isLoading = rolePending || assignPending || permsPending;

  // Derive which permission_ids are currently assigned to this role
  const roleId = Number(id);
  const originalAssignments = useMemo(
    () => allAssignments.filter((a) => a.role_id === roleId),
    [allAssignments, roleId]
  );
  ...
  const handleSave = async () => {
    ...
    const addCalls = toAdd.map((permId) =>
      rolePermissionsApi
        ...
        .then(() => ({ ok: true }))
        .catch((err) => {
          // 409 = already assigned by a concurrent change — treat as success
    ...
    const removeCalls = toRemove.map((assignment) =>
      rolePermissionsApi
