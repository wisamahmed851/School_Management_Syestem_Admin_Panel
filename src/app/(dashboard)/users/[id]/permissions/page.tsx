...
import { useUserPermissionsList } from "@/hooks/use-user-permissions";
...
export default function UserPermissionsPage() {
  ...
  const { data: allAssignments = [], isPending: assignPending } = useUserPermissionsList();
  const { data: allPermissions = [], isPending: permsPending } = usePermissionsList();

  const isLoading = userPending || assignPending || permsPending;
  ...
  const handleSave = async () => {
    ...
    const addCalls = toAdd.map((permId) =>
      userPermissionsApi
        .create({ user_id: userId, permission_id: permId })
        .then(() => ({ ok: true }))
        .catch((err) => {
    ...
    await qc.invalidateQueries({ queryKey: ["user-permissions"] });
    setSaving(false);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Select which permissions are directly assigned to this user.
          </p>
        </div>
